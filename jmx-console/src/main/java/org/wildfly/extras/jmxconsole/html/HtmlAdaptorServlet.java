...
public class HtmlAdaptorServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (action == null) action = DISPLAY_MBEANS_ACTION;

        if (action.equals(DISPLAY_MBEANS_ACTION)) displayMBeans(request, response);
        else if (action.equals(INSPECT_MBEAN_ACTION)) inspectMBean(request, response);
        else if (action.equals(UPDATE_ATTRIBUTES_ACTION)) updateAttributes(request, response);
        else if (action.equals(INVOKE_OP_ACTION)) invokeOp(request, response);
        else if (action.equals(INVOKE_OP_BY_NAME_ACTION)) invokeOpByName(request, response);
    }
    ...
    private void displayMBeans(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (filter != null && filter.length() > 0) {
            ...
            if (filter.charAt(filter.length() - 1) == '"') filter = filter.substring(0, filter.length() - 2);
    ...
    private void updateAttributes(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        while (paramNames.hasMoreElements()) {
            ...
            if (param.equals("name") || param.equals("action")) continue;
    ...
    private void invokeOpByName(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (methodName == null) throw new ServletException("No methodName given in invokeOpByName form");
        try {
