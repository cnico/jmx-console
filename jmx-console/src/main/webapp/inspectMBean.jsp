...
<table width="100%" cellspacing="0" cellpadding="0" border="0" align="center">
    <tr>
        <td height="105" align="center"><h1>JMX MBean View</h1><%= hostname %>
        </td>
        <td height="105" align="center" width="300" nowrap>
            <p>
                ...
                <input type="button" value="Refresh MBean View"
                       onClick="javascript:location='HtmlAdaptor?action=inspectMBean&amp;name=<%= URLEncoder.encode(request.getParameter("name"),"UTF-8") %>'"/>
            </p>
            <%= new java.util.Date() %>
        </td>
    </tr>
...
<table width="100%" cellspacing="1" cellpadding="1" border="1" align="center">
    ...
    <tr>
        <th>Java Class</th>
        <td colspan="2">
            <jsp:getProperty name='mbeanData' property='className'/>
        </td>
    </tr>
    <tr>
        <th>Description</th>
        <td colspan="2"><%= fixDescription(mbeanInfo.getDescription())%>
        </td>
    </tr>
</table>
            ...
            for (int a = 0; a < attributeInfo.length; a++) {
                ...
                if (attrInfo.isWritable()) {
                    ...
                    if (attrType.equals("boolean") || attrType.equals("java.lang.Boolean")) {
                        ...
                        if (String.valueOf(attrValue).indexOf(sep) == -1) {
                            out.print(
                                    "<input type='text' size='80' name='" + attrName + "' value='" + translateMetaCharacters(
                                            attrValue) + "' " + readonly + "/>");
                        } else {
                            out.print(
                                    "<textarea cols='80' rows='10' type='text' name='" + attrName + "' " + readonly + ">" + attrValue + "</textarea>");
                        }
                    } else {
                    ...
                    if (attrType.equals("[Ljavax.management.ObjectName;")) {
                        ...
                        if (names != null) {
                            for (int i = 0; i < names.length; i++) {
                                out.print(
                                        "<p align='center'><a href='HtmlAdaptor?action=inspectMBean&name=" + URLEncoder.encode(
                                                names[i] + "", "UTF-8") + ">" + names[i] + "</a></p>");
                            }
                        }
                ...
                if (attrType.equals("javax.management.ObjectName")) {
                    if (attrValue != null) {
                        out.print(
                                "<p align='center'><a href='HtmlAdaptor?action=inspectMBean&name=" + URLEncoder.encode(
                                        attrValue, "UTF-8") + "'>View MBean</a></p>");
                    }
                }
                out.println("    </pre>");
                out.println("   </td>");
